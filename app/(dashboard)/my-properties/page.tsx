import { createClient } from '@/lib/supabase/server'
import { requireRole, getCurrentUser } from '@/lib/auth'
import { PropertyCard } from '@/components/properties/property-card'
import { Building2 } from 'lucide-react'

export default async function MyPropertiesPage() {
  const user = await requireRole(['property_manager'])
  const supabase = await createClient()

  // Get properties assigned to this property manager
  const { data: assignments } = await supabase
    .from('property_managers')
    .select(`
      property:properties(*)
    `)
    .eq('user_id', user.id)

  const properties = assignments?.map((a: any) => a.property).filter(Boolean) || []

  // Get total enrollments for assigned properties
  const propertyIds = properties.map((p: any) => p.id)
  let totalEnrollments = 0
  
  if (propertyIds.length > 0) {
    const { count } = await supabase
      .from('enrollments')
      .select('*', { count: 'exact', head: true })
      .in('property_id', propertyIds)
    totalEnrollments = count || 0
  }

  return (
    <div className="px-8 py-8">
      <div className="mb-8">
        <h1 className="text-5xl font-normal text-beagle-dark">My Properties</h1>
        <p className="text-sm text-gray-600 mt-2">Properties assigned to you</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-200">
          <div className="flex items-center gap-3 mb-3">
            <Building2 className="h-8 w-8 text-beagle-orange" />
            <p className="text-sm font-medium text-gray-600">Assigned Properties</p>
          </div>
          <p className="text-4xl font-bold text-beagle-dark">{properties.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-200">
          <p className="text-sm font-medium text-gray-600 mb-3">Total Enrollments</p>
          <p className="text-4xl font-bold text-beagle-dark">{totalEnrollments}</p>
        </div>
      </div>

      {/* Properties Grid */}
      {properties.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {properties.map((property: any) => (
            <PropertyCard
              key={property.id}
              property={property}
              href={`/my-properties/${property.id}`}
            />
          ))}
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm p-12 text-center border border-gray-200">
          <p className="text-sm text-gray-600">
            You haven't been assigned to any properties yet. Contact an administrator for access.
          </p>
        </div>
      )}
    </div>
  )
}

